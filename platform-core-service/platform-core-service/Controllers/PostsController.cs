...
namespace platform_core_service.Controllers
{
    ...
    public class PostsController : ControllerBase
    {
        ...
        [HttpPost("share")]
        public async Task<IActionResult> CreateShare([FromBody] CreatePostShareDTO createDTO)
        {
            var returnResult = new ReturnResult<SelectPostDTO>();
            try
            {
                returnResult = await _postService.CreateShareAsync(createDTO);
            }
            catch (Exception ex)
            {
                DevNexusLogger.Instance.Debug(ex.Message);
        ...
        public async Task<IActionResult> GetPostAndQAByProfileId([FromBody] Page<string> page, [TrimmedRequired] string profileId)
        {
            ...
            try
            {
                returnResult = await _postService.GetPostAndQAByProfileId(page, profileId);
            }
            catch (Exception ex)
            {
                DevNexusLogger.Instance.Debug(ex.Message);
