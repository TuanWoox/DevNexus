...
import { Users, ShieldAlert, ArrowLeft, Home } from "lucide-react";
...
export default function CommunityNotFound({ isForbidden = false, message }: CommunityNotFoundProps) {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] px-4 py-16 text-center">
            ...
            <div className="relative mb-6 flex items-center justify-center">
                ...
                <div className="relative flex items-center justify-center rounded-full bg-destructive/10 p-6 text-destructive size-20 sm:size-24 border border-destructive/20 shadow-inner">
                    {isForbidden ? (
                        ...
                        <Users className="h-10 w-10 sm:h-12 sm:w-12 animate-pulse" />
                    )}
                </div>
            </div>
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold text-foreground tracking-tight max-w-md">
                {isForbidden ? "Access Restricted" : "Community Not Found"}
            </h1>
            <p className="mt-3 text-sm sm:text-base text-muted-foreground max-w-md leading-relaxed">
                {message || (isForbidden
                    ? "You don't have permission to view this community. It might be private or restricted to specific members."
                    : "The community you are looking for doesn't exist, has been deleted, or is currently unavailable.")}
            </p>
