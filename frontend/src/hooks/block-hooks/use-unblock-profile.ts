...
export function useUnblockProfile(blockId?: string | null, otherProfileId?: string) {
    ...
    return useMutation({
        mutationFn: (variables?: UnblockProfileVariables) => {
            const finalBlockId = variables?.blockId ?? blockId;
        ...
        onSuccess: (data, variables) => {
            if (data) {
                const finalBlockId = variables?.blockId ?? blockId;
                const finalProfileId = variables?.blockedProfileId ?? otherProfileId;
