...
export const useGetFollowingsByProfileId = (
    profileId: string,
    request: Omit<Page<string>, "pageNumber" | "size">,
    enabled = true
) => {
    return useInfiniteQuery({
        queryKey: [...userFollowQueryKeys.lists(), 'byProfile', profileId, 'followings', { filters: request }],
        queryFn: ({ pageParam = 0 }) =>
            userFollowService.getFollowingsByProfileId(profileId, {
                ...request,
